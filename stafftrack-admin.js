...
function getQrPayload(staff) {
  const payload = { id: staff.id, name: staff.name, dept: staff.dept, role: staff.role, app: 'StaffTrack' };
  if (scriptUrl) payload.api = scriptUrl;
...
function loadCloudData() {
  ...
  cloudSyncBusy = true;
  const cb = 'staffTrackCloud_' + Date.now() + '_' + Math.random().toString(36).slice(2);
    ...
    window[cb] = data => {
      try {
        if (data && data.ok) {
          ...
          logs = data.logs || [];
          schoolInfo = data.school || schoolInfo;
          attRules = data.rules || attRules;
...
async function refreshCloudAndRender() {
  ...
  if (active && active.id === 'page-setup') renderStaffList();
}

function updateSigninLink() {
  const link = document.querySelector('a[href^="signin.html"]');
  if (!link) return;
  link.href = scriptUrl ? 'signin.html?api=' + encodeURIComponent(scriptUrl) : 'signin.html';
}
...
function renderStaffList() {
  ...
  staffList.forEach((s, idx) => {
    ...
    setTimeout(() => {
      makeQR(qrTarget, getQrPayload(s), 100);
    }, 100 + idx * 80);
  });
}
...
function openQRModal(id) {
  ...
  setTimeout(() => {
    makeQR(modalQREl, getQrPayload(s), 200);
  }, 100);
}
...
async function saveScriptUrl() {
  ...
  updateConnStatus();
  updateSigninLink();
  await postCloud('saveAll', { staff: staffList, logs, pins: freshPins(), deviceSessions: freshDeviceSess(), school: schoolInfo, rules: attRules });
...
async function testConnection() {
  ...
  try {
    ...
    await postCloud('addLog', testData);
